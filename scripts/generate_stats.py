...
Outputs, all sharing one visual language with ascii.svg (the portrait):
  ...
  hd-*.svg    section headings, set in the same mono face
...
def draw_heading(word):
    ...
    only way to put the page's own typeface on it, matching ascii.svg and the
    stat graphics. The rule starts past the longest plausible advance (0.6em is
    the widest common monospace ratio), so a narrower font on the viewer's
    machine widens the gap slightly rather than colliding with the text.
    """
    FS = 16
    H = 26
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "eduardoaugustolb")
    ...
    for word in ("projeto", "stack", "motivos", "stats", "contato"):
